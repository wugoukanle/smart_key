...
可选菜单的结束范围是整个菜单的数量，
数组包括两个参数，一个是数据其实地址，另一个是数据量的个数，
这样的菜单设计，可以只需知道数组名，就可推出数组元素个数
...
MENU_ITEM SETMENU[MAX_MENU]={
    ...
    {32,4,"固件更新",firmware_upgrade},
		{48,6,"关于",version},//pfun
};
...
MENU_ITEM FIRMWARE_UPGRADE[MAX_MENU]={
		{4,4,"",0},
		{0,6,"确定",begin_upgrade},
		{96,6,"取消",0},
		{16,2,"确定更新吗？",0}
}; 


MENU_ITEM READ_MAC[MAX_MENU]={
		{4,4,"",0},
		{0,6,"读取",0},
		{96,6,"返回",0},
		{16,2,"蓝牙地址:",0}
...
*
*/
void ShowSlideMenu(const MENU_ITEM *Menu) //主菜单显示
{
    uint8_t i,j,idx;
	
		const uint8_t window_len=2;			//滚动显示区域的范围
		uint8_t window[window_len];			//存储滚动窗口菜单的序号
	
	
		uint8_t optional_start,optional_end,optional_num,optional_show_len;
		optional_start=Menu[0].row;			//可选菜单的起始序号
		optional_end=Menu[0].col;				//可选菜单的结束序号
		optional_num=optional_end-optional_start+1;   //可选菜单的数量
	
	  //f_CurSel=optional_start;
	
		//非可选菜单项显示，菜单项的设计时，非可选菜单在前，可选菜单在后，这样就可以知道整个菜单项的数量
    for (idx=1;idx<optional_start;idx++)
		...
		//确定窗口菜单项
		if(optional_num<=window_len)			                //如果可选菜单数小于等于窗口长度，即只有两个滚动菜单
		{
				for(i=0;i<optional_num;i++)
					window[i]=optional_start+i;
				optional_show_len=optional_num;								
		}
		else																							//如果可选菜单数大于窗口长度，滚动菜单数量大于等于3
		{
				if(f_CurSel>optional_end-window_len+1)				//特殊情况，f_CurSel在可选菜单尾部的可选窗口范围内时
				{
// 						//可选菜单区的尾后一部分
...
// 						}		
						...
						//可选菜单都是可选菜单尾部的可选窗口范围内菜单
					  for(i=0;i<window_len;i++)
					    window[i]=(optional_end-window_len+1)+i;						
				...
				{
						for(i=0;i<window_len;i++)				//将当前菜单项开始窗口长度的菜单项作为可选菜单
							window[i]=f_CurSel+i;
				}
		...
		//可选菜单显示，根据window[]数组中的序号来选取菜单项，显示的位置也要更新
    for (idx=0;idx<optional_show_len;idx++)
    {
       if (window[idx]==f_CurSel)
           OLED_ShowN(32, 2+idx*2, (uint8_t*)Menu[window[idx]].Name);		//会将选中的行反白显示
       else
           OLED_Show(32, 2+idx*2, (uint8_t*)Menu[window[idx]].Name);		//未选中的行正常显示
    }
		
		
		
// 		//可选菜单显示，根据window[]数组中的序号来选取菜单项
//     for (idx=0;idx<optional_show_len;idx++)
//     {
//        if (window[idx]==f_CurSel)
...
{
  uint8_t ss_flag=0,i=0,j=0,nChar,temp_f_CurSel,flagshijian=0,keycode;		//ss_flag即 ScreenSaver_flag
	uint8_t option_menu_min,option_menu_max;
	
	option_menu_min=HsMenu[0].row;		//HsMenu[0].row是最小
	option_menu_max=HsMenu[0].col;		//HsMenu[0].col是最大
	
  f_CurSel=option_menu_min;	
	
	//ResetKey();			//这里一定不能清除按键值
  ...
  while(g_WaitTimeOut == 0)
	{		
		...
		if(ss_flag==1&&(nChar ==KEYCLEAR||nChar ==KEYSURE||nChar ==KEYDOWN||nChar == KEYUP))
		{
		...
		Mask_Key_Interrupt();
// 		if(ss_flag==0&&nChar == KEYUP)	//上翻	++
// 		{
// 				i=0;

// 				if (f_CurSel>option_menu_min)				
// 						--f_CurSel;
// 				else   
// 						f_CurSel = option_menu_max;		  
// 				
// 				OLED_Clear();
...
// 				ResetKey();
// 		}
// 		else if(ss_flag==0&&nChar ==KEYDOWN)//下翻	--
// 		{
// 			i=0;
// 		
// 			if (f_CurSel<option_menu_max)
// 					++f_CurSel;
// 			else
// 					f_CurSel=option_menu_min;
// 			
...
// 		}
		...
		else if( nChar ==KEYCLEAR&&ss_flag==0)//取消
		{
			i=0;
...
// 			Main_Oled_Time();		
	...
	g_State =ST_IDLE;			//最重要，退出菜单时进入IDLE状态
...
{
  uint8_t ss_flag=0,i=0,j=0,nChar,temp_f_CurSel;
	uint8_t option_menu_min,option_menu_max;
	
	option_menu_min=TOOLMENU[0].row;
	option_menu_max=TOOLMENU[0].col;
  ...
  while(1)			//循环出口就在取消中
  {
		...
		if(ss_flag==1&&(nChar ==KEYCLEAR||nChar ==KEYSURE||nChar ==KEYDOWN||nChar == KEYUP))	//按任意键开屏
		{
			OLED_Display_On();
			ss_flag=0;
			ResetKey();
		...
		if(nChar == KEYUP)	//上翻	++
		{
			 ...
			 if (f_CurSel>option_menu_min)					//TOOLMENU[0].row=3最小
						--f_CurSel;
			 else															 			//TOOLMENU[0].row=3最小
						f_CurSel=option_menu_max;					//TOOLMENU[0].col=4最大
										
				 
			OLED_Clear();	
			ShowSlideMenu(TOOLMENU);		//此时f_CurSel已经改变，清屏之后再显示时反白的行会改变
			Main_Oled_Power();
			Main_Oled_Time();	
			ResetKey();	
		...
		else if(nChar ==KEYDOWN)//下翻	--
		{
			...
			if (f_CurSel<option_menu_max)			//TOOLMENU[0].col=4是最大
					++f_CurSel;
			else 
					f_CurSel=option_menu_min;			//TOOLMENU[0].row=3是最小
		...
		else if(nChar ==KEYSURE) //确定
		{
			...
			f_CurSel=temp_f_CurSel;		//f_CurSel表示当前的行
			ResetKey();
			ShowSlideMenu(TOOLMENU);	
			Main_Oled_Power();
			Main_Oled_Time();
		} 
		else if( ss_flag==0&&nChar ==KEYCLEAR)//取消
		{
			i=0;
...
// 	uint8_t ss_flag=0,j=0,nChar,temp_f_CurSel;
...
//   //InsertKey(KEYNO);   //在按键缓冲区，插入一个虚拟的初始化按键
// 	
// 	
// // 	while(1)
// // 	{
...
{
  uint8_t ss_flag=0,i=0,j=0,nChar, temp_f_CurSel=0;
	uint8_t option_menu_min,option_menu_max;
	
	option_menu_min=MIMA[0].row;
	option_menu_max=MIMA[0].col;
  f_CurSel =MIMA[0].row;
...
void Choose_MenuOp4_3(void)
{
  uint8_t ss_flag=0,i=0,j=0,nChar,temp_f_CurSel;
	...
	option_menu_min=SHUTDOWNKEY[0].row;
	option_menu_max=SHUTDOWNKEY[0].col;
...
void Choose_MenuOp4_4(void)
{
  uint8_t ss_flag=0,i=0,j=0,nChar,temp_f_CurSel;
	...
	option_menu_min=READ_MAC[0].row;
	option_menu_max=READ_MAC[0].col;
  f_CurSel =READ_MAC[0].row;

	ResetKey();
	
	OLED_Clear();	 
	ShowMenuNoInverse(READ_MAC);
	Main_Oled_Power();
	Main_Oled_Time();		
  ...
  while(1)
  {
		...
		if(nChar ==KEYSURE) //确定
		{
			...
			bluetooth_mac();		//将读到的MAC地址写入外部flash中，可以更新flash中的MAC地址
			
			SPI_FLASH_Read(MAC,BLUETOOTH_MAC,6);
			
			OLED_ShowNumHex(16,4,MAC[0],2,16);
			OLED_ShowNumHex(32,4,MAC[1],2,16);
			OLED_ShowNumHex(48,4,MAC[2],2,16);
			OLED_ShowNumHex(64,4,MAC[3],2,16);	
			OLED_ShowNumHex(80,4,MAC[4],2,16);
			OLED_ShowNumHex(96,4,MAC[5],2,16);			
			
			//for dubug
			delay_ms(1000);				
			UART1nSendChar(1,MAC,6);
			delay_ms(1000);		
...
/*void Choose_MenuOp4_4(void)
{
  uint8_t ss_flag=0,j=0,epi,epr,hangj,jj,k=0,nChar,temp_f_CurSel=0,tempdata[2],tempsuoma[4],recordsuoma[4];
...
if( ss_flag==0&&nChar ==KEYCLEAR)//取消
		{
			    i=0;
			    j++;
					//KeyNum =KEYNO;
...
void Choose_MenuOp5(void)
{
  uint8_t ss_flag=0,i=0,j=0,nChar,temp_f_CurSel;
	...
	option_menu_min=SETMENU[0].row;
	option_menu_max=SETMENU[0].col;
  f_CurSel =SETMENU[0].row;

	ResetKey();
	
	OLED_Clear();	    //清屏
	ShowSlideMenu(SETMENU);		//显示菜单
	Main_Oled_Power();
	Main_Oled_Time();	
  ...
  {		
		...
		if(nChar == KEYUP)	//上翻	++
		{
				...
				OLED_Clear();
				ShowSlideMenu(SETMENU);		//此时f_CurSel已经改变，清屏之后再显示时反白的行会改变
				Main_Oled_Power();
				Main_Oled_Time();						
				ResetKey();
		}
		else if(nChar ==KEYDOWN)//下翻	--
		{
				...
				if (f_CurSel<option_menu_max)			//HsMenu[0].col是最大
						++f_CurSel;
				else if(f_CurSel>=option_menu_max)
						f_CurSel=option_menu_min;		//HsMenu[0].row是最小
				
				OLED_Clear();
				ShowSlideMenu(SETMENU);
				Main_Oled_Power();
				Main_Oled_Time();						
...
void Choose_MenuOp5_1(void)
{
  uint8_t ss_flag=0,i=0,j=0,nChar,temp_f_CurSel;
	...
	option_menu_min=TIME[0].row;
	option_menu_max=TIME[0].col;
  f_CurSel=TIME[0].row;
...
void Choose_MenuOp5_2(void)
{
  uint8_t ss_flag=0,i=0,j=0,nChar,temp_f_CurSel;
	...
	option_menu_min=SETSUCCESS[0].row;
	option_menu_max=SETSUCCESS[0].col;
  f_CurSel =SETSUCCESS[0].row;
...
void Choose_MenuOp5_3(void)
{
  uint8_t ss_flag=0,i=0,j=0,nChar,temp_f_CurSel;
	...
	option_menu_min=VERSION[0].row;
	option_menu_max=VERSION[0].col;
...
*@功能：固件更新
*/
void Choose_MenuOp5_4(void)
{
  uint8_t ss_flag=0,i=0,j=0,nChar,temp_f_CurSel;
	uint8_t option_menu_min,option_menu_max;
	
	option_menu_min=FIRMWARE_UPGRADE[0].row;
	option_menu_max=FIRMWARE_UPGRADE[0].col;
  f_CurSel =FIRMWARE_UPGRADE[0].row;

	ResetKey();

	OLED_Clear();	    //清屏
	ShowMenu(FIRMWARE_UPGRADE);		//显示菜单
	Main_Oled_Power();
	Main_Oled_Time();		
...
// 	OLED_DrawBMP(96,0,128,2,OLED_buf_batt0);	
	...
	if(pc ==0)
	{
		OLED_DrawBMP(96,0,128,2,OLED_buf_batt0);	//满电量
	}
	else if(pc ==1)
	{
		OLED_DrawBMP(96,0,128,2,OLED_buf_batt1);
	}
	else if(pc ==2)
	{
		OLED_DrawBMP(96,0,128,2,OLED_buf_batt2);	
	}
	else if(pc ==3)
	{
		OLED_DrawBMP(96,0,128,2,OLED_buf_batt3);	//低电量
	}
	else //即pc==4
	{
		OLED_DrawBMP(96,0,128,2,OLED_buf_batt3);	//低电量  
...
// 	if((USB_CHEAK&(GPIO_Pin_1))!=0)  //USB_CHEAK ==(GPIO_Pin_1)
// 	{
// 		OLED_DrawBMP(96,0,127,15,lcd_buf_signal);
// 	}
// 	else if((USB_CHEAK&(GPIO_Pin_1))==0)
// 	{
