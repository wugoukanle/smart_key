...
void Ymodem_PreparePacket(uint8_t *SourceBuf, uint8_t *data, uint8_t pktNo, uint32_t sizeBlk)
{
    ...
    size = sizeBlk < packetSize ? sizeBlk :packetSize;
		//      sizeBLK<128时，packetSize=128，size=sizeBLK
		//128<=sizeBLK<1024时，packetSize=128，size=128
		//	  sizeBLK>=1024时，packetSize=1024，size=1024
    ...
    if ( size  <= packetSize)		//当size<128时具有补0x1A的作用
    {
...
/*******************************************************************************
  ...
  * @输入参数   data ：数据地址指针，data应该是构造好的数据包的指针
                 length：长度
  * @输出参数   无
  * @返回参数   无
*******************************************************************************/
void Ymodem_SendPacket(uint8_t *data, uint16_t length)
{
    ...
    while (i < length)
    {
        Send_Byte(data[i]);		//Send_Byte()是通过串口发送字节的函数
        i++;
    }
}
